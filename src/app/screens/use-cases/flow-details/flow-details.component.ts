...
export class FlowDetailsComponent implements OnInit {
  ...
  public ngOnInit() {
    this.parentItem.children.push(new Item({
      name: 'test1',
      children: [
        new Item({ name: 'Alternative Flow' }),
        new Item({ name: 'IF' }),
        new Item({ name: 'Loop Over' })
      ]
    }));
    this.parentItem.children.push(new Item({
      name: 'test2',
      children: [
        new Item({ name: 'IF' }),
        new Item({ name: 'Use Case Ends With Success' }),
        new Item({
          name: 'Exit Loop', children: [
            new Item({ name: 'Use Case Ends With Failure' })
          ]
        })
      ]
    }));
    this.parentItem.children.push(new Item({ name: 'Loop While' }));
  }
